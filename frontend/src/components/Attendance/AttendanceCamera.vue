<template>
  <div class="camera-container">
    ...
    <div class="camera-controls">
      ...
      <el-button 
        :disabled="!isPreviewing || isProcessing || isAutoMode"
        @click="handleCapture" 
        type="success"
...
const startFaceDetection = () => {
  ...
  detectionInterval = setInterval(async () => {
    if (!videoRef.value || isProcessing.value || !autoDetecting.value) return

    try {
      const result = await detectFace(videoRef.value)
      faceDetected.value = result.detected

      if (result.detected && !isProcessing.value && autoDetecting.value) {
        autoDetecting.value = false
        const imageBase64 = capturePhoto(videoRef.value)
        emit('photo-captured', imageBase64)

        setTimeout(() => {
          if (isAutoMode.value && isPreviewing.value) {
            autoDetecting.value = true
            faceDetected.value = false
          }
        }, 12000)
